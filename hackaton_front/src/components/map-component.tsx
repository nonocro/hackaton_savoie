...
async function fetchAllDoctors(): Promise<Doctor[]> {
	const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/departements/73/medecins`);
...
async function fetchAllCommunes(): Promise<Commune[]> {
	const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/departements/73/communes`);
...
function CommunePolygons({
	...
	const communeColors = useMemo(() => {
		...
		communes.forEach((commune) => {
			...
			if (population <= 0) {
				...
				if (ratio <= 0) {
					colors[commune.code_insee] = `rgba(${colorStops[0][0]}, ${colorStops[0][1]}, ${colorStops[0][2]}, 0.6)`;
				} else if (ratio < badRatio * 0.5) {
					const factor = ratio / (badRatio * 0.5);
					colors[commune.code_insee] = interpolateColor(colorStops[0], colorStops[1], factor);
				} else if (ratio < badRatio) {
					const factor = (ratio - badRatio * 0.5) / (badRatio * 0.5);
					colors[commune.code_insee] = interpolateColor(colorStops[1], colorStops[2], factor);
				} else if (ratio < (badRatio + goodRatio) / 2) {
					const factor = (ratio - badRatio) / ((goodRatio - badRatio) / 2);
					colors[commune.code_insee] = interpolateColor(colorStops[2], colorStops[3], factor);
				} else if (ratio < goodRatio) {
					const factor = (ratio - (badRatio + goodRatio) / 2) / ((goodRatio - badRatio) / 2);
					colors[commune.code_insee] = interpolateColor(colorStops[3], colorStops[4], factor);
				} else if (ratio < idealRatio) {
					const factor = (ratio - goodRatio) / (idealRatio - goodRatio);
					colors[commune.code_insee] = interpolateColor(colorStops[4], colorStops[5], factor);
				} else {
					const cappedRatio = Math.min(ratio, idealRatio * 1.5);
					const factor = (cappedRatio - idealRatio) / (idealRatio * 0.5);
					colors[commune.code_insee] = interpolateColor(colorStops[5], colorStops[6], Math.min(1, factor));
				}
			}
		});
	...
	const legend = useMemo(() => {
		...
		return (
			...
			>
				<div style={{ fontSize: '12px', fontWeight: 'bold' }}>Médecins pour 1000 habitants</div>
				<div style={{ position: 'relative' }}>
					<div style={gradientStyle}></div>
					<div style={{ position: 'relative', height: '15px', marginTop: '2px' }}>
						{markers.map((marker) => (
							<div
								key={marker.value}
								style={{
	...
	return (
		<>
			{communes.map((commune) => {
				const polygonCoords: LatLngExpression[] = commune.polygone.map(([lat, lng]) => [lat, lng]);

				if (polygonCoords.length === 0) return null;

				const doctorCount = doctors?.filter((d) => d.code_insee === commune.code_insee).length || 0;
				const doctorsPerThousand = commune.population ? ((doctorCount / commune.population) * 1000).toFixed(2) : 'N/A';
				...
				return (
					<Polygon
						...
						pathOptions={{
							color: '#333',
							weight: 1,
							fillColor: communeColors[commune.code_insee] || 'rgba(0, 0, 255, 0.1)',
						...
						{!isIsochroneActive && (
							<Popup>
								<div>
									...
									<p className="text-sm">Population: {commune.population.toLocaleString()}</p>
									<p className="text-sm">Médecins: {doctorCount}</p>
									<p className="text-sm">Médecins pour 1000 habitants: {doctorsPerThousand}</p>
									<p className="text-sm">
										{doctorCount > 0 && commune.population > 0
											? `1 médecin pour ${Math.round(commune.population / doctorCount).toLocaleString()} habitants`
											: 'Aucune couverture médicale'}
									</p>
									<p className="text-sm font-medium">Statut: {coverageStatus}</p>
								</div>
							</Popup>
						)}
					</Polygon>
...
function IsochroneControl({
	...
	return (
		...
		>
			<div style={{ marginBottom: '5px', fontSize: '12px', fontWeight: 'bold' }}>Zone d'accessibilité</div>
			<button
				...
				title={isActive ? 'Désactiver' : 'Cliquez pour activer le mode isochrone, puis cliquez sur la carte'}
			>
...
export function MapComponent({ location, isochroneTime, saisonTime }: Readonly<MapComponentProps>) {
	...
	const [isochroneCenter, setIsochroneCenter] = useState<LatLngLiteral | null>(null);
	...
	const handleIsochroneUpdate = (coords: number[][] | null, newCenter: LatLngLiteral) => {
		setIsochrone(coords);
	...
	return (
		<MapContainer center={center} zoom={10} style={{ height: '100%', width: '80%' }}>
			...
			<CommunePolygons communes={communes} doctors={doctors} saison={saisonTime} isIsochroneActive={isochroneActive} />
