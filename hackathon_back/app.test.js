...
const base_url_medecins = process.env.BDJSON
	? 'http://localhost:3001'
...
describe('API Express Tests', () => {
	...
	test('GET /departements/73/medecins?codePostal=73000 retourne des médecins', async () => {
		nock(base_url_medecins)
			.get('/medecins')
			.query(true)
			.reply(200, [{ nom: 'Jacob', address: 'une super adresse 73011' }]);
	...
	test('GET /departements/73/communes/73065 retourne une commune avec population', async () => {
		nock(base_url_communes)
			.get('/communes')
			.reply(200, [{ nom: 'Albertville', code: '73065', population: 10035 }]);
	...
	test('GET /recherche/medecins sans coordonnées retourne une erreur', async () => {
		nock(base_url_localisation)
			.get('/reverse')
			.query(true)
			.reply(200, { nom: 'Albertville', code: '73065', population: 10035, address: { postcode: '73250' } });
